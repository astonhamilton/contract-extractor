...
from packages.pipeline.procurement_context_stage import (
    ProcurementContextStageConfig,
    apply_procurement_context_run,
    execute_procurement_context_run,
)


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for canonical procurement-context staging/apply."""
    parser = argparse.ArgumentParser(
        description="Run staged procurement-context inference or apply a staged run into manifests."
    )
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Run procurement-context inference into a staged run folder without mutating manifests.",
    )
    mode.add_argument(
        "--apply-run",
    ...
    parser.add_argument(
        ...
        help="LiteLLM model name for procurement-context inference.",
    )
...
def resolve_run_report_path(apply_run: str) -> Path:
    ...
    return (
        ...
        / "data"
        / "processed"
        / "procurement_context_runs"
        / apply_run
        / "procurement_context_run_report.json"
    )


def main() -> None:
    """Run or apply canonical procurement-context stage."""
    configure_logging()
    ...
    config = ProcurementContextStageConfig(
        ...
        runs_dir=REPO_ROOT / "data" / "processed" / "procurement_context_runs",
        indexes_dir=REPO_ROOT / "data" / "processed" / "indexes",
        model=args.model,
        workers=args.workers,
    )

    print("Starting canonical procurement-context stage.")
    print(
    ...
    if args.dry_run:
        print("Mode: dry-run")
        print("This writes staged procurement-context artifacts under data/processed/procurement_context_runs/.")
        ...
        if args.doc_id:
            ...
            print("Selection: all eligible docs without canonical procurement context")

        report, report_path = execute_procurement_context_run(
            config=config,
            doc_id=args.doc_id,
        ...
        print(
            "Procurement-context dry-run complete: "
    ...
    print("Mode: apply")
    print("This copies staged procurement-context outputs into canonical derived folders and updates manifests.")
    print(f"Source run report: {run_report_path}")
    result = apply_procurement_context_run(
        config=config,
    ...
    print(
        "Procurement-context apply complete: "
