...
from packages.pipeline.classification_stage import (
    ClassificationStageConfig,
    apply_classification_run,
    execute_classification_run,
)
from packages.pipeline.logging_utils import configure_logging


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for canonical classification staging/apply."""
    parser = argparse.ArgumentParser(
        description="Run staged contract classification or apply a staged run into manifests."
    )
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Run classification into a staged run folder without mutating manifests.",
    )
    mode.add_argument(
        "--apply-run",
    ...
    parser.add_argument("--force", action="store_true", help="Include or overwrite already-classified docs.")
    parser.add_argument(
        "--model",
        default="openai/gpt-5.4-mini",
        help="LiteLLM model name for classification.",
    )
    parser.add_argument(
        "--reasoning-effort",
        default="medium",
...
def resolve_run_report_path(apply_run: str) -> Path:
    ...
    return (
        ...
        / "classification_runs"
        / apply_run
        / "classification_run_report.json"
    )


def main() -> None:
    """Run or apply canonical classification stage."""
    configure_logging()
    ...
    config = ClassificationStageConfig(
        ...
        runs_dir=REPO_ROOT / "data" / "processed" / "classification_runs",
    ...
    print("Starting canonical classification stage.")
    print(
    ...
    if args.dry_run:
        print("Mode: dry-run")
        print("This writes staged classification artifacts under data/processed/classification_runs/.")
        ...
        if args.doc_id:
            ...
            print("Selection: all eligible unclassified docs")

        report, report_path = execute_classification_run(
            config=config,
            doc_id=args.doc_id,
        ...
        print(
            "Classification dry-run complete: "
    ...
    print("Mode: apply")
    print("This copies staged classification outputs into canonical derived folders and updates manifests.")
    print(f"Source run report: {run_report_path}")
    result = apply_classification_run(
        config=config,
